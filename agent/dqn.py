...
class ReplayBuffer():
    ...
    def sample(self, batch_size):
        ...
        actions = torch.tensor(vals[1], dtype=torch.long, device=device)
        rewards = torch.tensor(vals[2], dtype=torch.float, device=device)
...
class BaseAgent(Base):
    def act(self, state, epsilon=0.0):
        ...
        if sample > epsilon:
            with torch.no_grad():
            # return int(np.argmax(self.forward(state).detach().numpy(), axis=1)[0])
                values, indices = torch.max(self.forward(state), 0)
...
def compute_loss(model, target, states, actions, rewards, next_states, dones):
    ...
    expected_state_action_values = next_state_values - (next_state_values * dones)
    expected_state_action_values = (expected_state_action_values * gamma) + rewards
